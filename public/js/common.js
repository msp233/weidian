...
$(function(){
    ...
    //         if(xhr.getResponseHeader('platform')=='app'){
    //             $('header').hide();
    //         };
    //     }
    // });
    ...
    //用cookie
    var arr=document.cookie.match(new RegExp("(^| )platform=([^;]*)(;|$)"));
    if(arr!=null){
         if(unescape(arr[2])=='app'){
             $('header').hide();
         };
    }
...
function localCache(name,data,exp) {
    //console.log(data,name)
    exp=exp||1;
    if(data){
        var _data = typeof(data) === 'string' ? data : JSON.stringify(data);
        localStorage.setItem(name,_data);
        localStorage.setItem(name+'_time',Date.now()+exp*24*60*60*1000)
    }else{
        var time = localStorage.getItem(name+'_time');
        var res;
        if(Date.now()-time>0) {
            return false
        }
...
function renderAppDonloadBar() {
    var whitelist = ['category.html','itemlist.html','item-list.html'];
    var page = window.location.href.match(/\/([^\.\/]+\.html)/);
    var arr=document.cookie.match(new RegExp("(^| )platform=([^;]*)(;|$)"));
    ...
    if(localCache('install_hnmall_app')){
        return false;
    }
    ...
    if(document.getElementById('J-call-app')){
        return false;
    }
    var body = document.getElementsByTagName('body')[0];
    var bar = document.createElement('div');
    bar.id='J-call-app';
    bar.style.cssText='height:50px;background-color: #ed674a;overflow:hidden;'
    var html = '<div class="bar" style="display: -webkit-flex;display: -ms-flexbox;display: flex;height: 100%;-webkit-align-items: center;-ms-flex-align: center;align-items: center;">\
                    <div class="logo" style="width:40px;height: 40px;margin:0 15px;background: url(https://www.hnmall.com/images/logo.jpg) no-repeat center;background-size: contain;"></div>\
                    <div class="grow" style="height:100%;color: #fff;-webkit-flex-grow: 1;-ms-flex-positive: 1;flex-grow: 1;">\
                        <div class="app-name" style="font-size: 15px;line-height:25px;">友阿微店APP</div>\
                        <div class="app-desc" style="font-size: 12px;line-height:14px;">002277友阿股份旗下新零售平台</div>\
                    </div>\
                    <div class="link" style="width: 100px;height: 32px;margin-right: 10px;text-align: center;line-height: 32px;background-color: #fff;border-radius: 5px;color: #ed674a;font-size: 14px;">立即打开</div>\
                </div>';
    bar.innerHTML = html;
    bar.addEventListener('click',function(){
        localCache('install_hnmall_app',1);
